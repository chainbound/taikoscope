...
/// Economics information for a single batch.
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchEconomicsItem {
    /// Batch ID.
    pub batch_id: u64,
    /// L1 block number where the batch was proposed.
    ...
    /// Total priority fees collected from all blocks in this batch.
    pub total_priority_fee: u128,
    /// Total base fees collected from all blocks in this batch.
    pub total_base_fee: u128,
    /// Total L1 data posting cost for this batch.
    pub total_l1_data_cost: Option<u128>,
    /// Net profit for this batch (priority + base fees - L1 costs).
    pub net_profit: i128,
    /// Number of transactions in all blocks of this batch.
    pub total_transactions: u64,
    /// Total gas used by all blocks in this batch.
    pub total_gas_used: u64,
    /// Timestamp when the batch was proposed.
    pub proposed_at: DateTime<Utc>,
}

/// Collection of batch economics data.
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchEconomicsResponse {
    /// Economics data for each batch.
    pub batches: Vec<BatchEconomicsItem>,
}

/// Profit information for a batch.
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchProfitItem {
    /// Batch ID.
    pub batch_id: u64,
    /// Net profit in wei (priority + base fees - L1 costs).
    pub net_profit: i128,
    /// L1 block number where the batch was proposed.
    pub l1_block_number: u64,
    /// First L2 block number in this batch.
...
/// Collection of batch profit rankings.
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchProfitRankingResponse {
    /// Profit rankings for the requested batches.
    pub batches: Vec<BatchProfitItem>,
}

/// Aggregated L2 fees for a sequencer (batch-level).
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchSequencerFeeRow {
    /// Sequencer address.
    pub address: String,
    /// Sum of priority fees across all batches by this sequencer.
    pub priority_fee: u128,
    /// Sum of base fees across all batches by this sequencer.
    pub base_fee: u128,
    /// Total L1 data posting cost across all batches by this sequencer.
    pub l1_data_cost: Option<u128>,
    /// Number of batches proposed by this sequencer.
    pub batch_count: u64,
}

/// Batch-level fee aggregation response.
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchL2FeesResponse {
    /// Sum of priority fees for the range (batch-level).
    pub priority_fee: Option<u128>,
    /// Sum of base fees for the range (batch-level).
    pub base_fee: Option<u128>,
    /// Total L1 data posting cost for the range (batch-level).
    pub l1_data_cost: Option<u128>,
    /// Fee breakdown for each sequencer (batch-level).
    pub sequencers: Vec<BatchSequencerFeeRow>,
}

/// Aggregated data for the main dashboard (batch-level).
#[derive(Debug, Serialize, ToSchema)]
pub struct BatchDashboardDataResponse {
    /// Average delay between L2 blocks in milliseconds.
    pub l2_block_cadence_ms: Option<u64>,
    /// Average delay between batch submissions in milliseconds.
    pub batch_posting_cadence_ms: Option<u64>,
    ...
    /// Average time to verify a batch in milliseconds.
    pub avg_verify_time_ms: Option<u64>,
    /// Average L2 transactions per second (calculated from batch data).
    pub avg_tps: Option<f64>,
    /// Latest preconfiguration data.
    pub preconf_data: Option<PreconfDataResponse>,
    /// Number of L2 reorg events in the selected range.
    ...
    /// Estimated infrastructure cost in USD for the requested range.
    pub cloud_cost: Option<f64>,
    /// Total number of batches in the range.
    pub total_batches: u64,
    /// Average blocks per batch.
    pub avg_blocks_per_batch: Option<f64>,
}
