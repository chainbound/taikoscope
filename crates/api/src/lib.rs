...
type BlobsPerBatchQuery = PaginatedQuery;
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Paginated blobs per batch", body = BatchBlobsResponse),
        (status = 500, description = "Database error", body = ErrorResponse)
    ),
    tag = "taikoscope"
)]
async fn blobs_per_batch(
    Query(params): Query<BlobsPerBatchQuery>,
    State(state): State<ApiState>,
...
>>>>>>> c800521 (feat(api): paginate blobs per batch)
