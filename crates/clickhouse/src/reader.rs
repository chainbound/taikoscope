...
impl ClickhouseReader {
    ...
    pub async fn get_batch_economics(&self, range: TimeRange) -> Result<Vec<BatchEconomicsItem>> {
        ...
        let query = format!(
            ...
            WITH batch_blocks AS (
                ...
                INNER JOIN {db}.l1_head_events l1_events
                    ON b.l1_block_number = l1_events.l1_block_number
                CROSS JOIN {db}.l2_head_events h
                WHERE l1_events.block_ts >= toUnixTimestamp(now64() - INTERVAL {interval})
                AND h.l2_block_number >= (b.last_l2_block_number - b.batch_size + 1)
            ...
            batch_costs AS (
                ...
                LEFT JOIN {db}.l1_data_costs dc ON bb.l2_block_number = dc.l2_block_number
            )
            SELECT
                bb.batch_id,
                bb.l1_block_number,
                bb.batch_size,
                bb.last_l2_block_number,
                bb.proposer_addr,
                sum(bb.sum_priority_fee) AS total_priority_fee,
                sum(bb.sum_base_fee) AS total_base_fee,
                sum(bc.cost) AS total_l1_data_cost,
                sum(bb.sum_tx) AS total_transactions,
                sum(bb.gas_used) AS total_gas_used,
                toUnixTimestamp(bb.inserted_at) AS proposed_at
            FROM batch_blocks bb
            LEFT JOIN batch_costs bc ON bb.batch_id = bc.batch_id
            GROUP BY bb.batch_id, bb.l1_block_number, bb.batch_size, bb.last_l2_block_number,
                     bb.proposer_addr, bb.inserted_at
    ...
    pub async fn get_batch_profit_ranking(
        ...
        let query = format!(
            ...
            FROM batch_blocks bb
            LEFT JOIN batch_costs bc ON bb.batch_id = bc.batch_id
            GROUP BY bb.batch_id, bb.l1_block_number, bb.last_l2_block_number, bb.batch_size, bb.proposer_addr
    ...
    pub async fn get_batch_l2_fees_by_sequencer(
        ...
        let query = format!(
            ...
            SELECT
                bb.proposer_addr,
                sum(bb.sum_priority_fee) as priority_fee,
                sum(bb.sum_base_fee) as base_fee,
                sum(bc.cost) as l1_data_cost,
                count(DISTINCT bb.batch_id) as batch_count
            FROM batch_blocks bb
            LEFT JOIN batch_costs bc ON bb.batch_id = bc.batch_id
            GROUP BY bb.proposer_addr
            ORDER BY priority_fee DESC",
            interval = range.interval(),
            filter = self.reorg_filter("h"),
            db = self.db_name,
    ...
    pub async fn get_batch_l2_fees_totals(
        ...
        let query = format!(
            ...
            FROM batch_blocks bb
            LEFT JOIN {db}.l1_data_costs dc
              ON bb.l2_block_number = dc.l2_block_number",
            interval = range.interval(),
            filter = self.reorg_filter("h"),
            sequencer_filter = sequencer_filter,
            db = self.db_name,
