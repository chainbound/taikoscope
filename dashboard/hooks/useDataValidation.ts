...
export const useDataValidation = ({
  ...
  const validationResult = useMemo(() => {
    ...
    const checks = [
      createDataCheck(timeRange, chartsData.blockTxData, 'Block Transactions'),
      createDataCheck(timeRange, chartsData.l2BlockTimeData, 'L2 Block Times'),
      createDataCheck(timeRange, chartsData.l2GasUsedData, 'L2 Gas Usage'),
      createDataCheck(timeRange, chartsData.secondsToProveData, 'Prove Times'),
      createDataCheck(
        timeRange,
  ...
  const getDataQualityScore = useMemo(() => {
    ...
    const validChecks = Object.values(chartsData).filter(
