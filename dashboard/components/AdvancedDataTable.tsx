...
import { exportTableData, ExportFormat, validateExportData } from '../utils/exportUtils';
...
export const AdvancedDataTable: React.FC<AdvancedDataTableProps> = ({
    ...
    useEffect(() => {
        if (cacheKey) {
            tableCache.set(stateKey, {
                searchTerm,
                filters,
                sortBy,
                sortDirection,
            }, undefined, 30 * 60 * 1000); // 30 minutes TTL
        }
    ...
    const hasActiveFilters = searchTerm || Object.values(filters).some(v => v) || sortBy;
    ...
    return (
        <div className="bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100 p-4 md:p-6 lg:p-8">
                                ...
                                <thead className="bg-gray-50 dark:bg-gray-800">
                                    <tr>
                                        {extraTable.columns.map((column) => (
                                            <th key={column.key} className="px-4 py-3 text-left text-sm font-medium text-gray-900 dark:text-gray-100">
                                                {column.label}
                                            </th>
                                        ))}
                                    </tr>
            ...
            <div className="mt-6 text-xs text-gray-500 dark:text-gray-400">
                <p>Keyboard shortcuts: ESC (back), Ctrl+F (search), Ctrl+E (export CSV), Ctrl+R (clear filters)</p>
            </div>
        </div>
    );
};